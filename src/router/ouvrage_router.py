...
async def recherche_unitaire(critere: str, val: str):
    with Session(ENGINE) as session:
        dic_critere = {
        ...
        for cle, valeur in dic_critere.items():
            if cle == critere:
                ouvrage_db = (session.query(Ouvrage).filter(valeur.contains(val))).all()
                return ouvrage_db
...
async def recherche_double(critere1: str, val1: str, critere2: str, val2: str):
    with Session(ENGINE) as session:
        dic_critere = {
            "auteur": Ouvrage.auteur_ouvrage,
            "catégorie": Ouvrage.categorie_ouvrage,
        ...
        for cle, valeur in dic_critere.items():
            if cle == critere1:
                query = query.filter(valeur.contains(val1))
            if cle == critere2:
                query = query.filter(valeur.contains(val2))
