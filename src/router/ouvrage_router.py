...
@router.patch(
    ...
    description="Changement du champs qui est nécessaire, les autres champs peuvent être supprimés et garderont leur ancienne valeur",
)
            ...
            if cle1 == critere1:
                ...
                for cle2, valeur2 in dic_critere["critere"].items():
                    ...
                    if cle2 == critere2:
                        ...
                        ouvrage_db = (
                            session.query(Ouvrage)
                            .filter(valeur1.contains(nom1))
                            .filter(valeur2.contains(nom2))
                        ).all()
                        return ouvrage_db
                    break
            break
